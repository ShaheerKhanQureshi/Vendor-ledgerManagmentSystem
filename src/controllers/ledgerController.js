...
// export const getAllCompaniesWithLedgers = async (req, res) => {
//   try {
//     // Fetching the most recent balance per vendor
//     const result = await executeQuery(`
//       SELECT 
//         v.id AS vendor_id,
//         v.company_name, 
...
//     // Transform balances to absolute values (positive only)
//     const companiesWithAbsoluteBalances = result.map(entry => ({
//       ...entry,
//       balance: Math.abs(entry.balance),  // Make balance positive
//     }));

//     // Calculate total debit, total credit, and total balance
...
//     res.status(200).json({
//       ledgerCount: result.length,
//       totalDebit: `PKR ${totalDebit.toFixed(2)}`,  // Format total debit in PKR
//       totalCredit: `PKR ${totalCredit.toFixed(2)}`,  // Format total credit in PKR
//       totalBalance: `PKR ${totalBalance.toFixed(2)}`,  // Format total balance in PKR
//       companies: companiesWithAbsoluteBalances,  // Return transformed companies with absolute balances
//     });
//   } catch (error) {
//     res.status(500).json({ message: "Error getting vendors with ledgers", error: error.message });
//   }
...
export const createLedger = async (req, res) => {
  const { vendor_name, date, challan_no, payment_method, descriptions, quantities, prices, units, debit, credit } =
    req.body
  ...
  try {
    ...
    const query = `
      INSERT INTO ledgers (vendor_name, date, challan_no, description, quantity, debit, credit, payment_method, price, unit)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `
...
export const updateLedger = async (req, res) => {
  const { id } = req.params;
  const { date, challan_no, descriptions, quantities, debit, credit, payment_method, vendor_name, prices, units } = req.body;
...
// export const updateLedger = async (req, res) => {
//   const { id } = req.params;
//    const { date, challan_no, descriptions, quantities, debit, credit, payment_method, vendor_name, prices, units } =
   // req.body
//   try {
//     if (!vendor_name || !date || !challan_no || !id) {
//       return res.status(400).json({ message: "Vendor name, date, challan number, and ID are required" });
